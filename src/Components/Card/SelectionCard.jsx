...
function SelectionCard({ data, currentPage, index }) {
  ...
  const handleSelectData = () => {
    ...
    if (currentData?.includes(data.id)) {
      ...
      // Remove persona_id when avatar is deselected
      if (currentPage === "avatarSelection" && data.persona_id && data.persona_id[0]) {
        const currentPersonas = selectedData["selectedPersona"] || [];
        const updatedPersonas = currentPersonas.filter((id) => id !== data.persona_id[0]);
        setSelectedData("selectedPersona", updatedPersonas);
      }
    } else {
      // Add item
      updatedData = [...currentData, data.id];
      // Add persona_id when avatar is selected
      if (currentPage === "avatarSelection" && data.persona_id && data.persona_id[0]) {
        const currentPersonas = selectedData["selectedPersona"] || [];
        const updatedPersonas = [...currentPersonas, data.persona_id[0]];
        setSelectedData("selectedPersona", updatedPersonas);
      }
    }
